...
import pandas as pd
from country_data import country_mapping  # Imports country mapping

# Mapping of gender, depression, marital status, and family size labels
...
# familysize_labels = ['2-3', '4-5', '6-7', '8+']
...
def create_table(selected_country):
    ...
    # If a specific country is selected, filter the DataFrame for that country
    ...
    # Initialize DataFrame to store percentages and mean
    data = {
        'Category': [
    ...
    # Add columns for depression levels and mean depression index
    ...
    # Calculate percentages and mean depression index for gender
    if 'gender' in df.columns and 'depression_level' in df.columns:
        ...
        for gender in gender_labels.values():
            if gender in depression_counts_gender.index:
                ...
                # Mean depression index for each gender
    ...
    # Calculate percentages and mean depression index for age group
    if 'age_group' in df.columns and 'depression_level' in df.columns:
        ...
        for age_group in age_labels:
            if age_group in depression_counts_age.index:
                ...
                # Mean depression index for each age group
    ...
    # Calculate percentages and mean depression index for marital status
    if 'married_status' in df.columns and 'depression_level' in df.columns:
        ...
        for married_status in married_labels.values():
            if married_status in depression_counts_married.index:
                ...
                # Mean depression index for each marital status
    ...
    # Calculate percentages and mean depression index for education
    if 'education_level' in df.columns and 'depression_level' in df.columns:
        ...
        for education_level in education_labels.values():
            if education_level in depression_counts_education.index:
                ...
                # Mean depression index for each education level
